...
while marginal rotators are shown as light blue points. Objects with
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=0.8\textwidth]{mcqbinarity}
  \caption{Histograms of the displacement off of a fiducial 3 Gyr solar
    metallicity isochrone for the \citet{McQuillan14} cool dwarfs
  (black), and the rapid rotator sample (red). }
  \label{fig:mcqbinarity}
\end{figure}
...
temperature calculated in \citet{Mathur17}. As before, we calculate the 
photometric excess above
a fiducial 3 Gyr isochrone. Because the full \citep{McQuillan14} sample does 
not have metallicity information, use use a single solar-metallicity
isochrone. As a result, there will be greater scatter between the main and 
multiple-star sequences. Nonetheless, it is clear that rapid rotators 
overwhelmingly cluster above the main sequence than the full sample.
...
spectra \citep{Badry18}. \citet{Badry18} analyzed all APOGEE spectra
...
\vzini{} objects with spectroscopy at higher resolution than APOGEE. If
