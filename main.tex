...
generating a population whose members differ only by metallicity and by
measuring the width of the single-star sequence imposed by the metallicity 
distribution. The population is generated by predicting the K-band luminosity
for the cool APOGEE dwarf sample using a 1 Gyr MIST isochrone, fixing
\Teff{} at 5000 K, but keeping the same metallicity, and subtracting the
median-metallicity isochrone. We also perform the same procedure, but apply the 
empirical metallicity correction in \cref{fig:met_trend} before subtracting the
median-metallicity isochrone. The resulting distribution of vertical
displacements is shown in \cref{fig:met_spread}, with the left figure having a
scatter of 0.14 dex and the right figure having a scatter of 0.10 dex. This
figure supports the finding that the empirical correction performed in
Section~\ref{sec:speccor} substantially reduced the width of the main sequence.
The primary reason for the reduction in the width of the main sequence is that
the low-metallicity objects are not permitted at large, positive vertical
displacements, which can be seen in the right panel of \cref{fig:metallicity}.
The empirical corrections cause the K-band magnitude to vary much more weakly
at low metallicities than predicted by the MIST isochrones. We note that the
models predict the bolometric corrections in the K-band behave similarly to
those in the V-band with metallicity.
...
\begin{figure*}[htb]
    ...
    \caption{\emph{Left:} Distribution of vertical displacements for a
        simulated 50\% binary 1 Gyr solar-metallicity MIST population with 90 K
        temperature uncertainties with respect to a 1 Gyr solar metallicity 
        isochrone. The intrinsic single star population is shown with a blue 
        histogram while the binary population is stacked on top in orange. The
        two populations are fit by a double-Gaussian where the fitted width of
        the single-star population is 0.087. The true width of the single-star
        population is 0.096.
        \emph{Right:} The fitted and intrinsic scatters of an ensemble of
        binary populations with temperature uncertainties ranging from 50 K to
        150 K. The best-fit line has a slope of 1.38 and a y-intercept of 
    0.01.}\label{fig:mswidth}
\end{figure*}

Despite the reduction in scatter, the measured scatter of the single-star
sequence is still too small for the scatter from the combined effects of
metallicity and temperature uncertainties. We test whether our method of
fitting a double-Gaussian distribution yields the correct intrinsic scatter in
the single-star sequence. We perform this test by forward-modeling a composite 
population with a 50\% binary fraction generated from 1 Gyr MIST isochrones 
with \Teff{} distributed uniformly between 4000-5250 K, and solar metallicity. 
We assume the binaries follow a flat mass-ratio distribution. We simulate our 
vertical displacements by adding a temperature errors with various widths
between 50--150 K and then subtracting a solar-metallicity MIST isochrone with 
the transformed temperature. A two-component Gaussian is then fitted to the
distribution, and the fitted width of the Gaussian centered on the single-star
sequence is considered the measured width. Simulations where the
double-Gaussian fit did not converge to a solution were excluded. The 
resulting distribution of vertical displacements for both binaries and single 
stars is shown in the left panel of \cref{fig:mswidth} for the case of a 90 K 
uncertainty in \Teff{}. The true width of the single-star sequence is
0.096 while the measured width is 0.087. The analysis for the full range of 
uncertainties is shown in the right panel of \cref{fig:mswidth}. The fitted and
instrinsic widths are highly correlated, and form a line with a slope of 1.38.
In order to correct for the fact that the double-Gaussian fit underestimates
the true width of the single-star main sequence, we multiply the fitted widths
we derive by 1.38.
...
After applying the correction from the previous section between the fitted and 
intrinsic scatter, the inclusive and conservative thresholds become -0.28 mag 
and -0.41 mag, respectively. To prevent age effects from biasing the results, 
we only performed statistics in the 4000--5250 K range.
...
binary. This rough calculation by itself contradicts with the observed 
...
cannot hold, or that there are selection effects in the sample.  
