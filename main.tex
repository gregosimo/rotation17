...
We use the iron-abundance as measured by APOGEE to act as a proxy for
metallicity. Although APOGEE provides a weighted metallicity [M/H], the stellar
models used in this work primarily use iron abundance as inputs, so we treat
[Fe/H] as our metallicity measure for this purpose. For typical APOGEE dwarfs
in our temperature range, the measured uncertainty is about 0.01 dex.

Given the stellar parameters from APOGEE, we use isochrones from the Dartmouth
Stellar Evolution Program (DSEP) \citep{Dotter07,Dotter08} to derive additional 
properties of dwarfs. Individual DSEP models are specified by an age,
metallicity, and alpha abundance. For each individual target, we select a DSEP
model using metallicities from APOGEE, assuming solar alpha abundances, and
setting the uncertainty to be between 1-14 Gyr. 

We classify the evolutionary state as well as binarity of our targets by
deriving absolute K-band magnitudes for the sample. K-band magnitudes offer the
advantage of being a relatively well-behaved proxy for luminosity, because it
is far into the Rayleigh-Jeans tail. Additionally, the extinction in the K-band
is extemely low.
...
exponentially-decreasing volume prior \cite{Berger18b}. Of the full probability
distribution that is calculated, we make use of the mode as well as the
1-\(\sigma\) upper and lower confidence intervals.


For all of our targets, we calculate radii using DSEP isochrones. 

\subsection{Rotation Data}

Our main measurement of rotation comes from \vsini{} values calculated by
APOGEE\@. The \vsini{} are measured similarly to the temperature and metallicity;
model spectra are convolved on a grid of various \vsini{} and minimized
simultaneously with the other parameters. Because \vsini{} is measured by
minimizing chi-squared on a grid, the pipeline does not report uncertainties or
upper limits.

In addition to the \vsini{} measured by APOGEE, we also make use of rotation
...
One effort to obtain high-resolution spectroscopic parameters for a sample of
asteroseismic subgiants calculated \vsini{} for a significant number of
...
\(\Teff \sim 6000 K\), we also compare to measurements of cool dwarfs in the 
Pleiades \citep{Stauffer87}. Because Pleiades dwarfs are significantly 
younger than those in the field,
...
validated. However, unlike \citet{Bruntt12}, the \citet{Stauffer87} survey 
reported a detection limit of 10 \kms, which is significantly below that in
\citet{Bruntt12}. As a result, this should largely test whether APOGEE
\vsini{} measurements experience severe systematics for dwarfs.
...
\begin{figure*}
    ...
    line indicates the \(\Teff = 5450\) K boundary.
    \emph{Right:} Magnitude excess of targets observed in the full dwarf sample
...
We select the APOGEE hot dwarf sample simply by from the
\textit{APOGEE2\_APOKASC\_DWARFh} flag. The temperatures and absolute K-band
magnitudes of the hot dwarf sample are shown in the left-hand panel of 
...
In order to more easily characterize our sample, we instead use targets
...
characterize due to reaching the subgiant branch by 14 Gyr. Because only a 
...
One of the main things we can notice is that most of the spectroscopic rapid 
rotators appear to be multiple-star systems. For the subsample of APOGEE
targets with \citep{McQuillan14} periods, the photometric rapid rotators also
seem to be multiple-star systems as well.

Given the two rotation measures in our sample, we also compare the
spectroscopic and photometric rapid rotator fractions.
...
A point-by-point comparison of rotation measures for these targets is shown 
...
To ensure that we weren't running into a problem with the methodology, we 
